...
BOOTSTRAP="${ROOT_DIR}/scripts/init-github.sh"

# The GitHub bootstrap must remain attached to the SSH terminal so the stable
# DNS mismatch prompt can wait and retry instead of exiting.
if grep -Eq "printf .*\|[[:space:]]*bash .*init\.sh" "$BOOTSTRAP"; then
  echo "init-github.sh must not pipe input into init.sh" >&2
  exit 1
fi
grep -Fq 'if [ ! -t 0 ]; then' "$BOOTSTRAP"

# GitHub bootstrap must not create or request temporary Basic Auth credentials.
if grep -Eq 'TEMP_AUTH|bootstrap.*password|WORKSPACE_AUTH_PROVIDER "nginx"' "$BOOTSTRAP"; then
  echo "init-github.sh must not use temporary Nginx authentication" >&2
  exit 1
fi

grep -Fq 'WORKSPACE_AUTH_PROVIDER "github"' "$BOOTSTRAP"
grep -Fq 'github nginx opencode' "$BOOTSTRAP"
grep -Fq 'bash "$RUNTIME_INIT"' "$BOOTSTRAP"
